...
function App() {
  ...
  // 새롭게 추가된 게임 규칙 생성 핸들러
  const handleGenerateRules = async (e) => {
    e.preventDefault();
    setLoadingRules(true);
    setErrorRules(null);
    setRules(null); // 이전 결과 초기화
    ...
    try {
      const response = await fetch('http://localhost:8000/generate-rules', {
        ...
        body: JSON.stringify({
          conceptId: parseInt(rulesConceptId), // conceptId를 정수로 변환하여 전송
        }),
      });
          ...
          <form onSubmit={handleGenerateRules} className="rules-form">
            <div className="form-group">
              ...
              <input
                type="number"
                id="rulesConceptId"
                value={rulesConceptId}
                onChange={(e) => setRulesConceptId(e.target.value)}
